...
export function saveOfflineTrack(episode, type = "video") {
  ...
  return RNFetchBlob.config({
    fileCache: true,
    path:
      RNFetchBlob.fs.dirs.CacheDir + `/episodes/${episode.id}-${type}.${suffix}`
  })
    ...
    .then(res => {
      ...
      console.log(
        "SUCCESS finishing download of offline episode " + episode.id
      );
      return { episode, type, offline_url, status: "ok" };
    })
    .catch(err => {
      console.log(
        "ERROR starting to download offline episode " + episode.id,
        err
      );
...
export function deleteCurrentlyDownloadingFiles(offlineEpisodes) {
  ...
  for (id in offlineEpisodes) {
    if (!!offlineEpisodes[id].videoUrl) {
      if (
        offlineEpisodes[id].videoStatus === offlineDownloadStatus.downloading
      ) {
        promises.push(
          removeOfflineTrack({ id: id }, offlineEpisodes[id].videoUrl)
        );
      }
    } else {
      if (offlineEpisodes[id].status === offlineDownloadStatus.downloading) {
        promises.push(
          removeOfflineTrack({ id: id }, offlineEpisodes[id].audioUrl)
        );
      }
    }
  }
