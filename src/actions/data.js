...
export function getChannels(user_id=0) {
    const request = DataApi.GetData('web.php', { fun: 'shows', user_id: user_id })
    return {
...
export function search(str,user_id,page = 1,perpage = 20,type="episode") {
    ...
    const request = DataApi.GetData('web.php', 
        { fun: 'search', str, user_id, page, perpage, type: types[type] });
    return {
      type: 'DATA_SEARCH',
      payload: request
    };
...
export function removeFavorite(user_id,channel_id,show_id) {
    ...
    const args = { fun: 'remove_favourite', user_id, show_id, video_id:channel_id };
    const request = DataApi.GetData("web.php", args );
    return {
...
export function deleteOfflineEpisode(episodeObject, offline_url, type="video") {
    const request = OfflineApi.removeOfflineTrack(episodeObject,offline_url,type);
    return {
